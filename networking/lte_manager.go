...
func AddLTEInterface(ifName string, connectionName string, apn string) error {
	log.Infof("[STUB]  would call\n")
	log.Infof(">nmcli con add type gsm ifname %s con-name %s apn %s\n", ifName, connectionName, apn)
	log.Infof("or equivalent\n")
	//Implementation TBD
...
func ConnectModem(index string, serial string, connectionName string, apn string) error {
	log.Infof("Connecting modem %s on srial interface %s with name % to APN %s\n",
