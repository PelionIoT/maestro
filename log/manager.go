...
type LoggingData struct {
	...
	StoredIfconfig []maestroSpecs.LogTarget
	// the settings as they are right now
	RunningIfconfig []maestroSpecs.LogTarget

	// last time the logging config was changed
	// ms since Unix epoch
	LastUpdated int64
...
func (logdata *LoggingData) MarshalJSON() ([]byte, error) {
	type Alias LoggingData
...
func (logdata *LoggingData) hasFlags() bool {
	return logdata.flagsSet
}

func (logdata *LoggingData) setFlags(v uint32) {
...
// initialized in init()
type loggingManagerInstance struct {
	// for persistence
	db              *bolt.DB
	networkConfigDB *stow.Store
	// internal structures - thread safe hashmaps
	byInterfaceName *hashmap.HashMap // map of identifier to struct -> 'eth2':&LoggingData{}
	...
	loggingConfig             []maestroSpecs.LogTarget
...
const DDB_LOGGING_CONFIG_CONFIG_GROUP_ID string = "log_group"

var instance *loggingManagerInstance

func (this *loggingManagerInstance) enableThreadCount() {
	this.threadCountEnable = true
}
func (this *loggingManagerInstance) incLogThreadCount() {
...
func (this *loggingManagerInstance) decLogThreadCount() {
	if this.threadCountEnable {
		this.interfaceThreadCountMutex.Lock()
		this.interfaceThreadCount--
...
func (this *loggingManagerInstance) waitForActiveLogThreads(timeout_seconds int) (wastimeout bool) {
	if !this.threadCountEnable {
		log.MaestroError("NetworkManager: Called waitForActiveLogThreads - threadCountEnable is false!")
		return
	}
...
func (this *loggingManagerInstance) StorageInit(instance storage.MaestroDBStorageInterface) {
	gob.Register(&maestroSpecs.NetIfConfigPayload{})
	gob.Register(&LoggingData{})
...
func (this *loggingManagerInstance) StorageReady(instance storage.MaestroDBStorageInterface) {
...
func (this *loggingManagerInstance) StorageClosed(instance storage.MaestroDBStorageInterface) {

}

func (this *loggingManagerInstance) SetInterfacesAsJson(data []byte) error {
...
func (this *loggingManagerInstance) GetInterfacesAsJson(enabled_only bool, up_only bool) ([]byte, error) {
	ifs := make([]*LoggingData, 0)
...
// func (inst *loggingManagerInstance) resetDNSBuffer() {
// 	inst.dnsResolvConf = bytes.NewBufferString(dnsTemplateFile)
// }

func newLoggingManagerInstance() (ret *loggingManagerInstance) {
	ret = new(loggingManagerInstance)
...
func GetInstance() *loggingManagerInstance {
	if instance == nil {
		instance = newLoggingManagerInstance()
...
func newIfData(ifname string, ifconf *maestroSpecs.NetIfConfigPayload) (ret *LoggingData) {
	ret = new(LoggingData)
	ret.IfName = ifname
...
func (this *LoggingData) setRunningConfig(ifconfig *maestroSpecs.NetIfConfigPayload) (ret *LoggingData) {
	ret.RunningIfconfig = new(maestroSpecs.NetIfConfigPayload)
...
//
func resetIfDataFromStoredConfig(netdata *LoggingData) error {
...
func updateIfConfigFromStored(netdata *LoggingData) (ok bool, err error) {
	if netdata.StoredIfconfig != nil {
		ok = true
	} else {
...
func (this *loggingManagerInstance) resetLinks() {
...
func (this *loggingManagerInstance) submitConfig(config *maestroSpecs.NetworkConfigPayload) {
	...
	for _, ifconf := range config.Interfaces {
		...
		var storedifconfig LoggingData
...
func (this *loggingManagerInstance) getIfFromDb(ifname string) (ret *LoggingData) {
...
func (this *loggingManagerInstance) getOrNewInterfaceData(ifname string) (ret *LoggingData) {
...
func (this *loggingManagerInstance) getInterfaceData(ifname string) (ret *LoggingData) {
	pdata, ok := this.byInterfaceName.GetStringKey(ifname)
	if ok {
		ret = (*LoggingData)(pdata)
	}
...
func (this *loggingManagerInstance) loadAllInterfaceData() (err error) {
	var temp LoggingData
	this.networkConfigDB.IterateIf(func(key []byte, val interface{}) bool {
		ifname := string(key[:])
		ifdata, ok := val.(*LoggingData)
		if ok {
...
func (this *loggingManagerInstance) DoesInterfaceHaveValidConfig(ifname string) (err error, ok bool, ifconfig *maestroSpecs.NetIfConfigPayload) {
...
func (this *loggingManagerInstance) SetInterfaceConfigByName(ifname string, ifconfig *maestroSpecs.NetIfConfigPayload) (err error) {
...
func (this *loggingManagerInstance) setupInterfaces() (err error) {
...
func (this *loggingManagerInstance) SetupExistingInterfaces() (err error) {
...
//SetupDeviceDBConfig
func (this *loggingManagerInstance) initDeviceDBConfig() {
...
func (this *loggingManagerInstance) SetupDeviceDBConfig() (err error) {
	...
	if this.ddbConnConfig != nil {
		...
		//Create a config analyzer object, required for registering the config change hook and diff the config objects.
		configLogAna := maestroSpecs.NewConfigAnalyzer(DDB_LOGGING_CONFIG_CONFIG_GROUP_ID)
		if configLogAna == nil {
			log.MaestroErrorf("NetworkManager: Failed to create config analyzer object, unable to fetch config from devicedb")
			err_updated := errors.New("Failed to create config analyzer object, unable to fetch config from devicedb")
			return err_updated
			...
			if err != nil {
				log.MaestroWarnf("LoggingManager: No logging config found in devicedb or unable to connect to devicedb err: %v. Let's put the current running config: %v.\n", err, *this.loggingConfig)
				err = this.ddbConfigClient.Config(DDB_LOGGING_CONFIG_NAME).Put(this.loggingConfig)
				if err != nil {
					log.MaestroErrorf("LoggingManager: Unable to put network config in devicedb err:%v, config will not be monitored from devicedb\n", err)
					err_updated := errors.New(fmt.Sprintf("\nUnable to put network config in devicedb err:%v, config will not be monitored from devicedb\n", err))
					return err_updated
				}
			} else {
				//We found a config in devicedb, lets try to use and reconfigure network if its an updated one
				log.MaestroInfof("LoggingManager: Found a valid config in devicedb [%v], will try to use and reconfigure network if its an updated one\n", ddbLoggingConfig)
				identical, _, _, err := configLogAna.DiffChanges(this.loggingConfig, ddbLoggingConfig)
				if !identical && (err == nil) {
					//The configs are different, lets go ahead reconfigure the intfs
					log.MaestroDebugf("LoggingManager: New network config found from devicedb, reconfigure nework using new config\n")
					this.loggingConfig = &ddbLoggingConfig
					this.submitConfig(this.loggingConfig)
					//Setup the intfs using new config
					this.setupInterfaces()
					//Set the hostname again as we reconfigured the network
					log.MaestroWarnf("LoggingManager: Again setting hostname: %s\n", this.ddbConnConfig.RelayId)
					syscall.Sethostname([]byte(this.ddbConnConfig.RelayId))
				} else {
					log.MaestroInfof("LoggingManager: New network config found from devicedb, but its same as boot config, no need to re-configure\n")
				}
			}
			//Since we are booting set the Network config commit flag to false
			log.MaestroWarnf("LoggingManager: Setting Network config commit flag to false\n")
			this.CurrConfigCommit.ConfigCommitFlag = false
			this.CurrConfigCommit.LastUpdateTimestamp = ""
			this.CurrConfigCommit.TotalCommitCountFromBoot = 0
			err = this.ddbConfigClient.Config(DDB_LOGGING_CONFIG_COMMIT_FLAG).Put(&this.CurrConfigCommit)
			if err != nil {
				log.MaestroErrorf("LoggingManager: Unable to put network commit flag in devicedb err:%v, config will not be monitored from devicedb\n", err)
				err_updated := errors.New(fmt.Sprintf("\nUnable to put network commit flag in devicedb err:%v, config will not be monitored from devicedb\n", err))
				return err_updated
			}
			...
			err, this.ddbConfigMonitor = maestroConfig.NewDeviceDBMonitor(this.ddbConnConfig)
			if err != nil {
				log.MaestroErrorf("LoggingManager: Unable to create config monitor: %v\n", err)
			} else {
				//Add config change hook for all property groups, we can use the same interface
				var networkConfigChangeHook NetworkConfigChangeHook

				//log target keys
				configLogAna.AddHook("target", networkConfigChangeHook)
				configLogAna.AddHook("levels", networkConfigChangeHook)
				configLogAna.AddHook("tag", networkConfigChangeHook)
				configLogAna.AddHook("pre", networkConfigChangeHook)
				configLogAna.AddHook("post", networkConfigChangeHook)
				configLogAna.AddHook("post-fmt-pre-msg", networkConfigChangeHook)

				//Add monitor for this config
				var origLoggingConfig, updatedLoggingConfig []maestroSpecs.LogTarget
				//Provide a copy of current network config monitor to Config monitor, not the actual config we use, this would prevent config monitor
				//directly updating the running config(this.networkConfig).
				origLoggingConfig = *this.loggingConfig

				//Adding monitor config
				this.ddbConfigMonitor.AddMonitorConfig(&origLoggingConfig, &updatedLoggingConfig, DDB_LOGGING_CONFIG_NAME, configLogAna)
				...
				var updatedConfigCommit ConfigCommit
				log.MaestroInfof("NetworkManager: Adding monitor for config commit object\n")
				this.ddbConfigMonitor.AddMonitorConfig(&this.CurrConfigCommit, &updatedConfigCommit, DDB_LOGGING_CONFIG_COMMIT_FLAG, configLogAna)
			}
		}
	} else {
		log.MaestroErrorf("NetworkManager: No devicedb connection config available, configuration will not be fetched from devicedb\n")
...
func (mgr *loggingManagerInstance) commitInterfaceData(ifname string) error {
	pdata, ok := mgr.byInterfaceName.GetStringKey(ifname)
	if ok {
		ifdata := (*LoggingData)(pdata)
...
func (this *loggingManagerInstance) doDhcp(ifname string, op maestroSpecs.NetInterfaceOperation) {
	var err error
	var newlease *DhcpLeaseInfo
	var ifdata *LoggingData
	var timeout time.Duration

	this.incLogThreadCount()
...
func (mgr *loggingManagerInstance) finalizePrimaryRoutes() {
...
func (mgr *loggingManagerInstance) watchInterface(ifname string) {
...
func (mgr *loggingManagerInstance) stopWatchInterface(ifname string) {
	mgr.watcherWorkChannel <- networkThreadMessage{
...
func (mgr *loggingManagerInstance) watchInterfaces() {
...
// TODO - loggingManagerInstance needs to be a TaskHandler... and Jobs should be started like any other Task
//
func (mgr *loggingManagerInstance) SubmitTask(task *tasks.MaestroTask) (errout error) {
	var err error
	...
	case maestroSpecs.OP_TYPE_NET_INTERFACE:
		...
		if ok {
			...
			if ifconfig != nil {
				...
				default:
					debugging.DEBUG_OUT("NETWORK>>>loggingManagerInstance.SubmitTask() unknown Op\n")
			...
			debugging.DEBUG_OUT("NETWORK>>>loggingManagerInstance.SubmitTask() not correct Op type\n")
			err := new(maestroSpecs.APIError)
			err.HttpStatusCode = 500
...
func (mgr *loggingManagerInstance) ValidateTask(task *tasks.MaestroTask) error {
	return nil
}
