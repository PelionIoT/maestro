...
# makes /tmp/maestroapi2.sock go to /tmp/maestroapi.sock - but dump all output which comes through

#SOCKET=/tmp/maestroapi2.sock
SOCKET=/tmp/maestroapi.sock

echo "Call------->"
echo curl -i \
    --unix-socket $SOCKET http://localhost/jobConfig/$ARG1/$ARG2 \
    -H "Host: 127.0.0.1" \
