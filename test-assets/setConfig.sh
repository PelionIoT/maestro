...
if [ -z "${ARG3}" ]; then
    echo "Usage $0 job-name config-name config-data"
    exit 1
fi
...
ARG3=${ARG3//\\/\\\\} # \ 
ARG3=${ARG3//\//\\\/} # / 
ARG3=${ARG3//\'/\\\'} # ' (not strictly needed ?)
ARG3=${ARG3//\"/\\\"} # " 
ARG3=${ARG3//   /\\t} # \t (tab)
ARG3=${ARG3//
/\\\n} # \n (newline)
ARG3=${ARG3//^M/\\\r} # \r (carriage return)
ARG3=${ARG3//^L/\\\f} # \f (form feed)
ARG3=${ARG3//^H/\\\b} # \b (backspace)

echo "Call------->"
PAYLOAD="{\"name\":\"${ARG2}\",\"job\":\"${ARG1}\",\"data\":\"${ARG3}\",\"encoding\":\"utf8\"}"
echo "Payload is -->"
echo $PAYLOAD
echo "<--"
echo curl -i \
